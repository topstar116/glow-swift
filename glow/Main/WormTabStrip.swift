...
public struct WormTabStripStylePropertyies {
    ...
    var kHeightOfWormForBubble:CGFloat = 30
...
public class WormTabStrip: UIView,UIScrollViewDelegate {
    ...
    ******************************************/
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        delegate?.TabDidScroll(scrollView: scrollView)
    ...
    public func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        let tab = tabs[currentTabIndex]
        setTabStyle()
        delegate?.WTSCurrentTab(index: tab.index!)
        adjustTopScrollViewsContentOffsetX(tab: tab)
    ...
    private func setWidthAndHeightOfWormForDistance(distance:CGFloat){
        if distance < 0
        {
            ...
            let _:CGFloat  = self.calculatePrespectiveHeightOfIndicatorLine(distance: distance)
            worm.frame.size.height = 30
            
            worm.frame.size.width = currentWormWidth
        }
        if eyStyle.wormStyel == .LINE {
                worm.frame.origin.y = eyStyle.kHeightOfTopScrollView - eyStyle.kHeightOfWorm
        }else{
//                worm.frame.origin.y = (worm.frame.origin.y)
        }
        
        
        worm.layer.cornerRadius = 10
    }
