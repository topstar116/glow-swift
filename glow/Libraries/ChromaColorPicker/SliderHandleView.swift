...
//        handleLayer.path = makeRoundedTrianglePath(width: bounds.width, height: bounds.height, radius: radius)
        handleLayer.path = UIBezierPath(ovalIn: self.bounds).cgPath
        handleLayer.strokeColor = UIColor.white.cgColor
        handleLayer.lineWidth = frame.width / 10 //4
        handleLayer.cornerRadius = 19
//        handleLayer.strokeColor = borderColor.cgColor
//        handleLayer.lineWidth = borderWidth
        handleLayer.position = CGPoint(x: 10, y: 0)
    }
